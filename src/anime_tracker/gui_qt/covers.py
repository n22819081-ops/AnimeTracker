...
class CoverImageCache(QObject):
    ...
    def request(self, url: str, token: Cancellation | None = None) -> QPixmap:
        if not url or (token is not None and token.is_cancelled()):return self.placeholder()
        ...
        if url not in self.pending:
            reply=self.network.get(QNetworkRequest(QUrl(url))); self.pending[url]=reply; self.pending_tokens[url]=token; reply.finished.connect(lambda url=url,reply=reply:self._finished(url,reply))
        return self.placeholder()

    def _finished(self,url,reply):
        self.pending.pop(url,None);token=self.pending_tokens.pop(url,None)
        if token is not None and token.is_cancelled():reply.deleteLater();return
    ...
    def cancel_pending(self):
        for reply in tuple(self.pending.values()):reply.abort()
