...
The default profile is `C:\AnimeTracker\Modern Anime Tracker\modern_profile_test`. On first launch it copies `Modern Anime Tracker\migration_test\anime_tracker_modern_v5.db` to `Modern Anime Tracker\modern_profile_test\data\anime_tracker_modern.db`. Both locations are development artifacts excluded from Git; the live `Legacy Anime Tracker\data\anime_tracker.db` is never selected as the modern writable database.
