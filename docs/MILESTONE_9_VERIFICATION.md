# Milestone 9 Verification

Version/build: `1.0.0` / `1.0.0-rc1`; schema 6. PyInstaller 6.21.0 produced a windowed 223-file onedir build with version/icon resources and Qt platform, image, style, network, Schannel TLS, SQLite, Python OpenSSL, requests, DPAPI, and notification dependencies. Inno Setup 6.7.3 compiled the per-user installer. The portable ZIP contains 223 valid members.

`pytest`: **707 passed, 47 subtests passed, 0 failed in 45.52s**. The focused user-reported regression suite passed 22/22 and the packaged-release suite passed 18/18. Disposable source-runtime validation passed for clean schema, configurable read-only Jellyfin roots, diagnostics, all 12 pages, scheduled SUCCESS/partial/offline/lock states, backup/restore, explicit adoption/source retention, candidate-free review actions, explicit candidate confirmation, active-profile isolation, live snapshot persistence, candidate regeneration, review reconciliation, cancellation/backoff, cover-request cancellation, evidence presentation, resize performance, native-Python installer selection, hostile-PATH Qt TLS isolation, and the production-profile copy. Automatic notification delivery stayed Stage 1.

## AniList search, related entries, and manual season mapping repair

AniList title search was sending unused `year`, `format`, and `season` variables explicitly as null. The live AniList endpoint returned zero media for that request shape even though a minimal title query returned valid results. Search now omits unused filters; a live verification returned AniList 97766 (`GAMERS!`) and 177754 (`We are GAMERS!!!!`). Search-result caching also now establishes the parent AniList metadata identity before writing cache rows, preventing the production foreign-key failure that the GUI had mislabeled as temporary unavailability. AniList manga URLs receive a specific anime-only validation message.

Add Selected now persists only explicitly confirmed AniList identities, including checked related entries. The Franchises page displays cached untracked sequels, prequels, side stories, movies, OVAs, and specials with an explicit add action. Review Server Match now offers an inventory-backed folder chooser with exact path and season scope. Manual selection creates a persistent season-specific mapping and never writes to Jellyfin. Normal unmapped entries no longer retain stale `No Jellyfin match found` review state after a complete scan.

The rebuilt onedir application passed packaged smoke testing with 12 pages and 69 active titles on a disposable profile. Release-specific tests passed 18/18. The production database remained SHA-256 `D2C95C8B0DF91789D7A63A0380CB4E8DD0B129B75FEEBEA0B3767EB7F4BD85C0`.

## Existing-profile detection acceptance fix

The initial package built `PROJECT_PRODUCTION_PROFILE` from `Path("C:")`, producing drive-relative `C:AnimeTracker\production_profile`. Detection therefore depended on the current directory and failed from the installed application directory. The corrected runtime normalizes the system drive to `C:\` before joining path components, now yielding absolute `C:\AnimeTracker\Modern Anime Tracker\production_profile` without relying on source files, environment-specific working directories, bootstrap presence, or executable location.

Read-only validation now confirms `integrity_check=ok`, zero foreign-key violations, schema 6, 69 active titles, 421 archived/orphaned records, 1,312 baselines, eight reviews, one mapping, 11 rejections, 27 candidates, and redacted disabled DPAPI references. Tests confirm both first-run actions enable, invalid profiles show a reason, opening first run performs no writes, project-local mode loads 69 titles/all 12 pages, and adoption preview shows source/target/counts. Production database/bootstrap/settings hashes remained unchanged.

Microsoft Defender on 2026-08-04 found no threats in distribution, installer, or ZIP (engine `1.1.26070.7`, product `4.18.26060.3008`, signatures `1.457.1.0`). Privacy audit passed. Security audit passed statically, at source runtime, and through packaged GUI acceptance.

## Matching-review and live-scan acceptance fix

The review dialog's Mark Not on Server button was not connected to any application action. The corrected path validates the selected review and AniList identity, writes an idempotent manual decision, resolves only that review, recalculates tracker/server/review state, records history, refreshes all pages, and surfaces failures. It does not require a candidate ID, mapping, confidence, target, or path.

Production scanning previously ended after persisting `inventory_snapshots`. It now generates and persists candidates, reapplies rejections/manual decisions, revalidates confirmed mappings, stores coverage, reconciles migrated reviews, updates page models, and displays real summaries. The full disposable live-root run completed in 6.67 seconds with 587 items, 12,335 files, 10,843 media files, 13 suggestions across 12 titles, one mapping revalidated, zero auto-confirmations, and zero integration failures. Two obsolete ambiguity reviews with no current candidate were superseded; genuine legacy mapping reviews remained.

See `MILESTONE_9_REVIEW_ACTION_FIX.md` for affected tables, lifecycle details, and acceptance evidence.

## Explicit mapping actions and coverage repair

Three production controls rendered normally but had no controller connection: Confirm Suggestion, Review Server Match in anime details, and View Franchise. Confirm Suggestion now passes the exact selected candidate ID and AniList ID to a production operation, reconstructs the latest complete persisted inventory snapshot, validates the cached AniList identity, confirms the season-scoped mapping, recalculates coverage, updates tracker state, refreshes every page, and surfaces failures without closing the review. Reject Candidate now persists a candidate rejection through the same active-profile guard.

Finished titles with a current candidate can open a synthetic explicit-review context even when an obsolete migrated review says no match was found. This exposed Bakemonogatari's existing `Bakemonogatari (2009) - Season 01` suggestion instead of requiring a new scan. The Coverage page now uses known AniList episode counts for expected/aired values and lists all 587 folders from the latest complete inventory snapshot in By server folder, not only confirmed mappings.

Disposable-profile acceptance confirmed The Dangers in My Heart Season 2 to Season 02, Bakemonogatari to Season 01, and Beautiful Bones to Season 01. All three produced complete coverage, `On Server` tracker state, and no remaining open review in the disposable copy. Installed-GUI acceptance opened both Dangers and Bakemonogatari review dialogs and displayed the full server-folder inventory; both dialogs were canceled without writing production mappings. The production database SHA-256 remained `27DB6560CCDF4736E0004CD75673337344A9631CA462135C5E348CD30D595B06`.

## Refresh and matching presentation repair

The Refresh All Active crash was caused by the Qt worker's private token adapter lacking `wait(timeout)`. One shared cancellation protocol now covers AniList refresh, rate-limit backoff, retry delay, inventory scan, GUI workers, and scheduled checks. The installed final package completed a real refresh against a disposable 69-title profile: 69 succeeded, zero failed, 24 cache hits, 47 network requests, and zero metadata changes. A clear completion dialog was shown.

Matching evidence is prose in the normal table and structured JSON only in the optional technical view. The scorer remains unbounded and additive, so the UI says `Match points`, not Score or percent. Thresholds are documented in `MATCHING_SCORE_RULES.md`. Season-scoped targets include `Season 02`; the selected details show parent folder, scope, episodes, confidence, points, and conflicts before confirmation.

The review dialog prepares its model and evidence once. Interactive fixed initial widths, elision, and a separate details panel eliminate resize-triggered fitting, wrapping, formatting, or database access. The automated 13-candidate resize test rebuilt neither evidence nor model and stayed below its performance ceiling.

PyInstaller's upstream QtNetwork hook began blocking indefinitely in its build-time `QSslSocket.supportsSsl()` capability probe. A project-local hook retains standard QtNetwork dependency collection while skipping only that probe. The final 223-file distribution contains the Schannel TLS backend and Python/OpenSSL runtime; release tests verify both.

An installed-system error exposed a second form of the same collision: Qt's OpenSSL backend discovered `C:\msys64\ucrt64\bin\libssl-3-x64.dll` through a temporary process `PATH`, after another incompatible crypto runtime was already loaded. Anime Tracker now removes only conflicting MSYS2/Cygwin/MinGW TLS directories from its own GUI process, explicitly selects Windows Schannel for cover-image networking, and excludes Qt's unused OpenSSL TLS plugin from the package. `Install.ps1` also resolves a native CPython 3.10+ executable explicitly and rejects POSIX-toolchain Python. No global environment variable is changed. Source and installed smoke tests pass with the conflicting MSYS2 directory deliberately placed first in `PATH`, and the installed executable exits `0` without an entry-point dialog.

The user confirmed that the packaged 1.0.0 first-run screen launches on the real Windows desktop. This acceptance pass also installed the final build over the existing per-user installation and exercised Refresh All Active and Review Server Match through the real packaged GUI using a disposable profile. No clean VM was available.

## User-reported runtime regression repair — 2026-08-15

The installed August 5 build reproduced four independent problems. Movies included every movie regardless of server state, so Fragtime appeared in Movies and On Server at once. Open review cases were shown per database row, creating three Beautiful Bones entries. The manual folder filter reduced correctly to one Beautiful Bones folder but did not select it or enable confirmation. AniList's HTTP 403 service-shutdown response was flattened into the misleading message “AniList rejected the request.” In addition, stale cached `nextAiringEpisode` values were displayed as current episode facts.

The rebuilt application makes the navigation categories consistent, groups simultaneous review reasons by profile and AniList identity, preserves all underlying review IDs for resolution, auto-selects a unique folder-filter result, and uses the cached retrieval time rather than rewriting freshness on cache fallback. Expired schedules now say `Last known Ep … · cached schedule expired`. AniList's current upstream message is surfaced as a temporary API shutdown while explicitly stating that the search is valid and preserving the entered text.

Notifications now contains an explicit title-only `New on Jellyfin` composer. It lists confirmed On Server titles, offers a local preview, requires a second confirmation before delivery, uses the DPAPI-protected shared credential, records draft/delivery status, sends no paths or scan details, and does not enable automatic notifications. Verification used a mocked adapter; no live Discord message was sent.

The installed package was replaced successfully with exit code 0 while the production database hash remained unchanged. Real installed-GUI acceptance confirmed Fragtime absent from Movies, one combined Beautiful Bones review containing all three reasons, the unique Beautiful Bones folder selected with `Use Selected Folder` enabled, truthful AniList outage text, expired-schedule labels, and the shared-chat composer. All mapping dialogs were canceled without saving. A clean disposable packaged profile passed all 12-page smoke checks with `C:\msys64\ucrt64\bin` first in the parent PATH and no crypto popup. The 223-file distribution retains `qcertonlybackend.dll` and `qschannelbackend.dll`; `qopensslbackend.dll` is absent. Microsoft Defender reported no threats in the rebuilt distribution and release directory.

Jellyfin media, the legacy task/database, automatic production Discord activation, and the separate Storage Checker were untouched. Milestone 10 was not started.
