...
Display titles follow one policy: English, Romaji, native, stored legacy title, then AniList ID only as an emergency fallback. AniList ID remains secondary technical context. `No confirmed server mapping` means there is no active mapping; `Unknown coverage` is reserved for mapped records without sufficient episode evidence.
